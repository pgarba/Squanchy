...
/*
static cl::opt<bool> EnableConcreteInterpreter(
    "souper-constant-synthesis-use-concrete-interpreter",
    cl::desc("Use concrete interpreter in constant synthesis (default=false)"),
    cl::init(false));

static cl::opt<unsigned> MaxSpecializations(
    "souper-constant-synthesis-max-num-specializations",
    cl::desc("Maximum number of input specializations in constant synthesis "
             "(default=15)."),
    cl::init(15));
*/

bool EnableConcreteInterpreter = false;
unsigned MaxSpecializations = 15;
}  // namespace

namespace souper {

Inst *getUBConstraint(Inst::Kind K, unsigned OpNum, Inst *C, InstContext &IC);

Inst *getConstConstraint(Inst::Kind K, unsigned OpNum, Inst *C,
                         InstContext &IC);

void addComplexConstraints(Inst *I, Inst *&Constraints,
                           const std::set<Inst *> &ConstSet, InstContext &IC);
...
static void visitConstants(Inst *I, std::set<Inst *> &Visited,
  ...
  if (Visited.insert(I).second) {
    if (AvoidNops) addComplexConstraints(I, Constraints, ConstSet, IC);
    unsigned OpNum = 0;
    for (auto Op : I->Ops) {
      if (ConstSet.find(Op) != ConstSet.end()) {
        Constraints = IC.getInst(
            Inst::And, 1,
            {Constraints, AvoidNops ? getConstConstraint(I->K, OpNum, Op, IC)
...
std::error_code SouperConstantSynthesis::synthesize(
    SMTLIBSolver *SMTSolver, const BlockPCs &BPCs,
    const std::vector<InstMapping> &PCs, InstMapping Mapping,
    std::set<Inst *> &ConstSet, std::map<Inst *, llvm::APInt> &ResultMap,
    InstContext &IC, unsigned MaxTries, unsigned Timeout, bool AvoidNops) {
        ...
        TriedAnte =
            IC.getInst(Inst::And, 1,
                       {IC.getInst(Inst::Eq, 1,
                                   {getInstCopy(Mapping.LHS, IC, InstCache,
                                                BlockCache, &VCCopy, true),
  ...
  visitConstants(Mapping.RHS, Visited, ConstConstraints, ConstSet, IC,
                 AvoidNops);
  ...
  for (int I = 0; I < MaxTries; ++I) {
    ...
    // TriedAnte /\ SubstAnte
    Inst *FirstQueryAnte = IC.getInst(
        Inst::And, 1,
        {ConstConstraints, IC.getInst(Inst::And, 1, {SubstAnte, TriedAnte})});

    std::string Query =
        BuildQuery(IC, BPCs, PCs, InstMapping(Mapping.LHS, Mapping.RHS),
                   &ModelInstsFirstQuery, FirstQueryAnte, true, true);

    if (Query.empty()) return std::make_error_code(std::errc::value_too_large);

    // EC = SMTSolver->isSatisfiable(Query, IsSat, ModelInstsFirstQuery.size(),
    //                              &ModelValsFirstQuery, Timeout);
    EC =
        SaturnSC.isSatisfiableCached(Query, IsSat, ModelInstsFirstQuery.size(),
                                      &ModelValsFirstQuery, Timeout);
    ...
    for (unsigned J = 0; J != ModelInstsFirstQuery.size(); ++J) {
      if (ConstSet.find(ModelInstsFirstQuery[J]) != ConstSet.end()) {
        ...
        ConstMap.insert(std::pair<Inst *, llvm::APInt>(ModelInstsFirstQuery[J],
                                                       Const->Val));
    ...
    std::map<Block *, Block *> BlockCache;
    Inst *RHSCopy =
        getInstCopy(Mapping.RHS, IC, InstCache, BlockCache, &ConstMap, false);
    ...
    EC = SaturnSC.isSatisfiableCached(Query, IsSat,
                                       ModelInstsSecondQuery.size(),
                                       &ModelValsSecondQuery, Timeout);
        ...
        if (Var->Name == BlockPred && !ModelValsSecondQuery[J].isZero())
          for (auto B : Blocks)
            for (unsigned I = 0; I < B->PredVars.size(); ++I)
              if (B->PredVars[I] == Var) B->ConcretePred = I + 1;

        if (ConstSet.find(Var) == ConstSet.end()) {
          SubstConstMap.insert(
              std::pair<Inst *, llvm::APInt>(Var, ModelValsSecondQuery[J]));
          VC.insert(
              std::pair<Inst *, llvm::APInt>(Var, ModelValsSecondQuery[J]));
        }
      }
      ...
      if (EnableConcreteInterpreter) {
        ...
        if (!LHSV.hasValue()) {
          llvm::report_fatal_error(
              "the model returned from second query evaluates to poison for "
              "LHS");
        }
        ConcreteLHS = IC.getConst(LHSV.getValue());
      } else {
        ConcreteLHS = getInstCopy(Mapping.LHS, IC, InstCache, BlockCache,
                                  &SubstConstMap, true);
      }

      SubstAnte = IC.getInst(
          Inst::And, 1,
          {IC.getInst(
               Inst::Eq, 1,
               {ConcreteLHS, getInstCopy(Mapping.RHS, IC, InstCache, BlockCache,
                                         &SubstConstMap, true)}),
           SubstAnte});
    }
  }
...
std::error_code SouperConstantSynthesisZ3::synthesize(
    SMTLIBSolver *SMTSolver, const BlockPCs &BPCs,
    const std::vector<InstMapping> &PCs, InstMapping Mapping,
    std::set<Inst *> &ConstSet, std::map<Inst *, llvm::APInt> &ResultMap,
    InstContext &IC, unsigned MaxTries, unsigned Timeout, bool AvoidNops) {
  ...
  for (int I = 0; I < MaxTries; ++I) {
    ...
    //    if (EC) {
    //      if (DebugLevel > 3)
    //        llvm::errs() << "ConstantSynthesis: solver returns error on first
    //        query\n";
    //      return EC;
    //    }
